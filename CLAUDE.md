...
**13 Tables (all prefixed with `remote_agent_`):**
...
10. **`users`** - User accounts for JWT authentication (username, password hash, role: admin|user)
11. **`project_members`** - Junction table: user-codebase access grants (roles: owner, member)
12. **`node_states`** - Per-node execution state for DAG resume and quality gate results
13. **`test_results`** - Parsed test suite results linked to node states (gate evidence)
