...
/**
 * Upsert a node state row. Throws on DB error — callers use fire-and-forget `.catch()`.
 */
...
/**
 * Create a test result row. Throws on DB error — callers use fire-and-forget `.catch()`.
 */
...
function safeJsonParse<T>(value: unknown, fallback: T): T {
  ...
  try {
    ...
    getLog().warn({ raw: value.slice(0, 200) }, 'db.json_parse_failed');
    return fallback;
  }
}
...
function normalizeNodeStateRow(row: NodeStateRow): NodeStateRow {
  return {
    ...
    gate_results: safeJsonParse(row.gate_results, []),
  };
}
...
function normalizeTestResultRow(row: TestResultRow): TestResultRow {
  return {
    ...row,
    failures: safeJsonParse(row.failures, []),
  };
}
