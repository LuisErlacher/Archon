...
function makeStore(overrides: Partial<IWorkflowStore> = {}): IWorkflowStore {
  return {
    ...
    upsertNodeState: mock(async () => {}),
    getNodeState: mock(async () => null),
    getNodeStates: mock(async () => []),
    getValidatedNodeOutputs: mock(async () => new Map<string, string>()),
    createTestResult: mock(async () => {}),
    getTestResults: mock(async () => []),
